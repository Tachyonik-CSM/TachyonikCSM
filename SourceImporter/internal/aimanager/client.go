...
// the import rules and their generated routines, the per-module AI assignment
// and system prompt, and the AI provider entries a rule may name to override the
// module default.
package aimanager
...
// Client is an HTTP client for the AIManager internal API. The request
// plumbing lives in tachyonik/lib/restclient; what remains here is the wire types
// and one line per endpoint.
type Client struct {
...
// GetAIByName fetches an AI entry by exact name from AIManager.
...
// ImportRule represents an import rule from AIManager
...
// ListImportRulesResponse represents the list import rules API response
type ListImportRulesResponse struct {
	ImportRules []ImportRule `json:"importRules"`
	Total       int          `json:"total"`
}

// GetImportRules fetches the effective import rules (one per type, highest version,
// with a passed active routine) from AIManager.
func (c *Client) GetImportRules() ([]ImportRule, error) {
	var rulesResp ListImportRulesResponse
	if err := c.rc.JSON("GET", "/api/internal/import-rules", nil, &rulesResp, http.StatusOK); err != nil {
		return nil, err
	}
	return rulesResp.ImportRules, nil
}

// GetImportRuleByID fetches a single import rule by ID via internal API
func (c *Client) GetImportRuleByID(id int64) (*ImportRule, error) {
	var rule ImportRule
	path := fmt.Sprintf("/api/internal/import-rules/%d", id)
	if err := c.rc.JSON("GET", path, nil, &rule, http.StatusOK); err != nil {
		return nil, err
	}
	return &rule, nil
}
...
// ClearGenerateRequest clears the generate_requested_at flag on an import rule
...
func (c *Client) ClearGenerateRequest(ruleID int64, reason string) error {
	...
	path := fmt.Sprintf("/api/internal/import-rules/%d", ruleID)
